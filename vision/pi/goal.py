...
class Processor:
	...
	def get_blue_hue(self, img):
		...
		blue_vals = [100, 168, 168] # gimp: [240, 92.1568627451, 92.1568627451]
		upper = [110, 255, 255] # gimp: [280, 100, 100]
		lower = [95, 117, 117] # gimp: [200, 78.431372549, 78.431372549]
		# basically divide by 2 for H and divide by 100/255 for S & V
...
def getDistFromCenter():
	...
	fout = open('Axis_Example.jpg', 'wb')
	fout.write(pic)
	fout.close()

	#read img into opencv format
	img = cv2.imread('Axis_Example.jpg')
	GlobalWidth = 640
	GlobalHeight = 480
	...
	if showPrint: cv2.imshow('blue_test', blue_only)
	if showPrint: cv2.imshow('goal_test', goals_img)
	if showPrint: print 'got blue'
	...
	if (idx != -1):
		...
		if showPrint: print "dist:", dist, "x:", x1, "y:", y1, "wall:", dist_to_wall
...
if __name__ == '__main__':
	...
	picurl = 'http://www.roborealm.com/FRC2013/Axis_Example.jpg'
	...
	if (debugging == False):
		...
		while True:
			try:
				...
				cv2.putText(goals_img, result, (5, 25),cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,255,255))
